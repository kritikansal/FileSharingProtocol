	...
	MD5_CTX *mdContext;
{
	...
	for (i = 0; i < 16; i++)
	{
		sprintf(ff4,"%02x",mdContext->digest[i]);
		strcat(abc,ff4);
	}
	//printf("%s",abc);
	//return abc;
...
{
	char revbuf[LENGTH];
	char fr_name[200] = "/home/kriti/work/tempp/t1/";
	char nn[100];
	strcpy(nn,st1);
	strcat(fr_name,st1);
...
void Download(int client_sock,char * st1)
{
	//printf("Endjkjhev\n");
	//char fs_name[20] ;
	char fs_name[200] = "/home/kriti/work/tempp/t1/";
	strcat(fs_name,st1);
	//scanf("%s" , fs_name);
...
int main(int argc , char *argv[])
{
	...
	while(1)
	{
		...
		recvfrom(client_sock,client_message,1000,0,(struct sockaddr *)&server, &addr_len);
	...
	//puts("bind done");
		if(strlen(client_message)>0)
		{
			...
			//printf("%s",messagesend);
			sendto(client_sock, messagesend, 8000, 0,(struct sockaddr *)&server, sizeof(struct sockaddr));
	...
	//	}
		...
		if(strlen(newww)>0)
		{
			sendto(client_sock, newww, 1024, 0,(struct sockaddr *)&server, sizeof(struct sockaddr));
		...
		else
		{
			...
			{
				recvfrom(client_sock,message,8000,0,(struct sockaddr *)&server, &addr_len);
